...
CREATE TABLE tb_map (
    ...
    factory_id VARCHAR(36) NULL COMMENT '厂区ID（重要：用于按厂区筛选地图）',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    created_by VARCHAR(100) NULL COMMENT '创建人',
    updated_by VARCHAR(100) NULL COMMENT '更新人',
...
CREATE TABLE tb_robot (
    ...
    factory_id VARCHAR(36) NULL COMMENT '厂区ID（重要：用于按厂区筛选机器人）',
...
CREATE TABLE tb_gimbalhistory (
    ...
    view_status VARCHAR(20) NULL COMMENT '查看状态：pending-未查看, viewed-已查看, processed-已处理（有报警信息时使用，无异常时为空）',
    inspection_result_status VARCHAR(20) NULL COMMENT '巡检结果状态：null-尚未结束（任务进行中）, normal-正常（任务完成且无异常）, warning-预警报警, critical-严重报警, emergency-危机报警',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    created_by VARCHAR(100) NULL COMMENT '创建人',
    updated_by VARCHAR(100) NULL COMMENT '更新人',
...
CREATE TABLE tb_task (
    ...
    total_duration INT NULL COMMENT '任务总用时（分钟）（重要：用于任务列表和详情显示）',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    created_by VARCHAR(100) NULL COMMENT '创建人',
    updated_by VARCHAR(100) NULL COMMENT '更新人',
...
CREATE TABLE tb_item (
    ...
    robot_id VARCHAR(36) NULL COMMENT '关联机器人ID（重要：用于按机器人筛选巡检项目）',
    ...
    FOREIGN KEY (device_id) REFERENCES tb_device(id) ON DELETE CASCADE,
    FOREIGN KEY (robot_id) REFERENCES tb_robot(id) ON DELETE SET NULL,
...
CREATE TABLE tb_itemhistory (
    ...
    process_status VARCHAR(20) NULL COMMENT '查看状态：pending-未查看, viewed-已查看, processed-已处理（有报警信息时使用，无异常时为空）',
    inspection_result_status VARCHAR(20) NULL COMMENT '巡检结果状态：null-尚未结束（任务进行中）, normal-正常（任务完成且无异常）, warning-预警报警, critical-严重报警, emergency-危机报警',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    created_by VARCHAR(100) NULL COMMENT '创建人',
...
CREATE TABLE tb_alarm_rule_relation (
    ...
    relation_type ENUM('item', 'gimbal', 'sensor', 'robot') NOT NULL COMMENT '关联类型',
...
CREATE TABLE tb_alarm_info (
    ...
    source_type ENUM('itemhistory', 'gimbalhistory', 'sensorhistory', 'robot_status') NOT NULL COMMENT '数据源类型',
    source_ids JSON NOT NULL COMMENT '数据源ID列表，按关联表顺序排列，格式：["id1", "id2", "id3"]',
    relation_type ENUM('item', 'gimbal', 'sensor', 'robot', 'none') NULL COMMENT '关联对象类型',
