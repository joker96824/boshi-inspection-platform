...
class GimbalTaskRepository:
    ...
    async def get_by_id(self, task_id: str) -> Optional[GimbalTask]:
        """根据ID获取云台任务"""
        query = select(GimbalTask).where(GimbalTask.id == task_id, GimbalTask.is_deleted == False)
    ...
    async def get_by_ids(self, task_ids: List[str]) -> List[GimbalTask]:
        ...
        query = select(GimbalTask).where(
            GimbalTask.id.in_(task_ids),
    ...
    async def get_all(self, page: int = 1, size: int = 20, 
                     task_name: str = None, task_type: str = None, gimbal_id: str = None) -> Tuple[List[GimbalTask], int]:
        """获取云台任务列表"""
        skip = (page - 1) * size
        ...
        if task_type:
            conditions.append(GimbalTask.task_type == task_type)
    ...
    async def get_by_gimbal_id(self, gimbal_id: str) -> List[GimbalTask]:
        """根据云台ID获取所有云台任务"""
        query = select(GimbalTask).where(
            GimbalTask.gimbal_id == gimbal_id,
            GimbalTask.is_deleted == False
        ).order_by(GimbalTask.created_at.desc())
        result = await self.db.execute(query)
        return list(result.scalars().all())
