...
class DualPTZRepository:
    ...
    async def get_by_robot_id(self, robot_id: str) -> Optional[DualPTZ]:
        """根据机器人ID获取双光云台配置（单个，兼容旧接口）"""
        stmt = select(DualPTZ).where(
            DualPTZ.robot_id == robot_id,
            DualPTZ.is_deleted == False
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    
    async def get_all_by_robot_id(self, robot_id: str) -> List[DualPTZ]:
        """根据机器人ID获取所有双光云台配置（列表）"""
        stmt = select(DualPTZ).where(
            DualPTZ.robot_id == robot_id,
            DualPTZ.is_deleted == False
        ).order_by(DualPTZ.created_at.asc())
        result = await self.db.execute(stmt)
        return list(result.scalars().all())
    ...
    async def hard_delete_by_robot_id(self, robot_id: str) -> int:
        """真删除指定机器人的所有双光云台配置（返回删除数量）"""
        from sqlalchemy import delete
        stmt = delete(DualPTZ).where(
            DualPTZ.robot_id == robot_id
        )
        result = await self.db.execute(stmt)
        await self.db.commit()
