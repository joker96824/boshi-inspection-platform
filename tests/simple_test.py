...
class SimpleTester:
    ...
    def test_auth_module(self):
        ...
        if success and status == 200:
            self.log_test("获取用户信息", True, f"用户: {data['data']['user']['username']}")
        else:
    ...
    def test_user_module(self):
        ...
        if success and status == 200:
            created_user_id = data["data"]["user"]["id"]
    ...
    def test_map_module(self):
        ...
        if success and status == 200:
            ...
            if success and status == 200:
                self.log_test("获取地图详情", True, f"地图名称: {data['data']['map']['map_name']}")
            else:
    ...
    def test_point_module(self, map_id: str):
        """测试巡检点管理模块"""
        print("\n📍 测试巡检点管理模块...")
    ...
    def test_log_module(self):
        ...
        # 3. 测试搜索日志
        success, data, status = self.make_request(
            "POST", "/api/v1/logs/search?keyword=admin&start_date=2025-09-22&end_date=2025-09-22&log_type=app", headers=headers
        )
        
        if success and status == 200:
            results = data["data"]["search_results"]
            total_matches = data["data"]["total_matches"]
            self.log_test("搜索日志", True, f"找到 {total_matches} 条匹配记录")
        else:
    ...
    def run_all_tests(self):
        ...
        # 5. 巡检点管理测试
        if map_id:
            self.test_point_module(map_id)
        
        # 6. ROS2模块测试
        self.test_ros2_module()
        
        # 7. 日志管理测试
        self.test_log_module()
